...
import { SkillArtifactPanel, skillArtifacts } from "@/components/skill-example";
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white border-4 border-black rounded-3xl p-6 md:p-10">
          ...
          {methodology.hasSkill && methodology.skillEffect && (
            <div className="mb-10 rounded-2xl border-4 border-[#4ECDC4] bg-[#4ECDC4]/10 p-6 md:p-7">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-full border-2 border-black bg-[#4ECDC4] px-3 py-1 text-xs font-bold text-black">
                  已做成可执行 Skill
                </span>
                <span className="text-sm text-gray-600">不只是一篇方法论文档，已固化成能安装、能自动跑的 Skill</span>
              </div>
              <p className="text-gray-800 leading-relaxed">
                <span className="font-bold">跑一次产出：</span>
                {methodology.skillEffect}
              </p>
            </div>
          )}

          {methodology.hasSkill && skillArtifacts[methodology.slug] && (
            <div className="mb-10">
              <SectionHeading title="这个 Skill 真实跑一次的成品" />
              <SkillArtifactPanel slug={methodology.slug} />
            </div>
          )}
